...
RAD_Writer::RAD_Writer(const Header &header, const Tag_Defn &tag_defn,
                       const Tag_List &file_tag_vals,
                       const std::string &op_file_path,
                       const std::size_t writer_count,
                       const std::size_t buf_cap)
    : output(op_file_path, std::ios::out | std::ios::binary), header(header),
      tag_defn(tag_defn), read_c_in_buf_(writer_count, 0), worker_c(0) {
  buf_.reserve(writer_count);
  for (std::size_t i = 0; i < writer_count; ++i)
    buf_.emplace_back(Buffer(buf_cap, output));

  FlushableBuffer fb(buf_cap * 10, output);
  header.write(fb);
  tag_defn.write(fb);
  fb.add(file_tag_vals);

  fb.flush();
  // now fb can go away and we'll use the other regular buffers
}
...
void RAD_Writer::flush_chunk(const std::size_t w_id) {
  ...
  output.write(reinterpret_cast<const char *>(&read_c_in_buf),
               sizeof(read_c_in_buf));
  buf.flush();
  lock.unlock();
